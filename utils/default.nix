...
# { pkgs }: let
#   toCamelCase = s: let
#       parts = pkgs.pkgs.lib.splitString "-" s;
#       upperFirst = word: 
#         (pkgs.pkgs.lib.toUpper (builtins.substring 0 1 word)) + (builtins.substring 1 (builtins.stringLength word) word);
#     in if (builtins.length parts) < 2 then s 
#     else let
#         first = pkgs.pkgs.lib.head parts;
#         rest = map upperFirst (pkgs.pkgs.lib.tail parts);
#     in builtins.concatStringsSep "" ([first] ++ rest);
# in {
#   packages = pkgs.pkgs.lib.mapAttrs' (fileName: _: 
#     let
#       baseName = pkgs.pkgs.lib.removeSuffix ".nix" fileName;
#       camelName = toCamelCase baseName;
#       importedFile = import (./. + "/${fileName}");
#       
#       args = if builtins.isFunction importedFile 
...
#              else {};
# 
#       # The wrapper that injects pkgs/stdenv into your "run.nix" style files
#       value = if args ? pkgs then
#         (overrides: importedFile (({
#           inherit pkgs;
#           stdenv = pkgs.stdenv;
#         } // (builtins.intersectAttrs args pkgs)) // overrides))
#       else
...
# 
#     in
#     pkgs.pkgs.lib.nameValuePair camelName value
#   ) pkgs.pkgs.lib.filterAttrs (name: type: 
#     type == "regular" && pkgs.pkgs.lib.hasSuffix ".nix" name && name != "default.nix"
#   ) (builtins.readDir ./.);
# }
